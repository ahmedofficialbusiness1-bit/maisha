...
const formSchema = z.object({
  username: z.string().min(3, {
    message: 'Jina la mtumiaji lazima liwe na angalau herufi 3.',
  }),
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (e.code === 'auth/email-already-in-use') {
        setError('Anwani hii ya barua pepe ishatumika.');
      } else {
      ...
      <Card className="w-full max-w-sm bg-gray-800/60 border-gray-700 text-white">
        <CardHeader>
          ...
          <CardDescription className="text-gray-400">
            Fungua akaunti mpya ili uanze kucheza.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
               {error && (
                <Alert variant="destructive">
                    ...
                    <AlertTitle>Hitilafu ya Kujisajili</AlertTitle>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Jina la Mtumiaji</FormLabel>
                    <FormControl>
                      <Input
